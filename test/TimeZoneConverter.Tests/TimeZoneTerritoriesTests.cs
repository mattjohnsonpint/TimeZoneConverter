...
public class TimeZoneTerritoriesTests
{
    ...
    {
        Assert.True(TZConvert.IanaTimeZoneNamesByTerritory.TryGetValue("US", out var zoneIds));
    ...
    {
        Assert.True(TZConvert.IanaTimeZoneNamesByTerritory.TryGetValue("US", out var usaZoneIds));
        Assert.NotEmpty(usaZoneIds);

        foreach (var territory in TZConvert.IanaTimeZoneNamesByTerritory.Keys)
        {
            if (territory == "US")
                continue;

            Assert.True(TZConvert.IanaTimeZoneNamesByTerritory.TryGetValue(territory, out var territoryZoneIds));
            Assert.NotEmpty(usaZoneIds);
